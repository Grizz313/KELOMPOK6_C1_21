<?php
  ...
  //  $barang = query("SELECT * FROM goods");
   $tampil = mysqli_query($koneksi, "SELECT * FROM users order by id_users asc");
    ...
    if (isset($_POST["cari"])) {
        $tampil = mysqli_query($koneksi, "SELECT * FROM users WHERE 
                               username LIKE '%".$_POST['keyword']."%' OR
                               nama LIKE '%".$_POST['keyword']."%' OR
                               no_hp LIKE '%".$_POST['keyword']."%' 
                               ");
...
<body>
  ...
  <div class="tabel col-md-10" style=" margin-left: 150px;">
        <h2 class="alert alert-dark text-center mt-5" style="background-color: #28a745";>ADMIN DATA DISPLAY</h2>
        <table rules="rows" class="table table-dark mt-2">
          <a href="create_admin.php" class="btn btn-success">ADD DATA</a>
          <thead>
              <tr>
              <th scope="col">No</th>
          ...
          <tbody>
              ...
              <tr>
                  ...
                  <td><?=$data['password'] ?></td>
                  <td>
                    <a href="update_admin.php?id=<?=$data['id_users'] ?>" class="btn btn-success">Edit</a>
                    <a href="delete_admin.php?id=<?=$data['id_users'] ?>" class="btn btn-danger">Delete</a>
                  </td>
              </tr>
          <?php $i++; ?>
          <?php endwhile; ?>
